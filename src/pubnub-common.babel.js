...
let PRESENCE_HB_THRESHOLD = 5;
let PRESENCE_HB_DEFAULT = 30;
const SDK_VER = version;
 ...
 */
function generate_channel_group_list(channelGroups, nopresence) {
  var list = [];

  _each(channelGroups, (channelGroup, status) => {
    if (nopresence) {
      if (channelGroup.search('-pnpres') < 0) {
        if (status.subscribed) {
          list.push(channelGroup);
        }
      }
    } else {
      if (status.subscribed) {
